...
#### The current script is tested on a sample of photon events. Before performing calibration using any of the regression libraries (mentitoned above), the cluster layer wise pt is corrected with layer weights obtained using bounded least square method.
#### This is a dev version of the BDTCalibration_photonsfinal.py script. It is the version used for doing studies for looking at custom learning rates (evolving) for the XGBOOST model.
...
tuningXGBOOST = 'False' # set to True if performing hyperparameter scan for XGBOOST
lossfuncdir = 'test' # huber or ls or final (output directory names used for storing plots 
...
dir_in2 = '/data_CMS_upgrade/ahuja/HGCAL/L1Trigger/ntuples/v3150b/RelValV10/hdfoutput/RelValDiG_Pt10To100_Eta1p6To2p9_'+pileup+'_mixedfe/'
...
paramGBR['loss']='lad' #huber/ ls / lad
...
for name in df_eg_train:
    ...
    if(tuningGBR == "True"):
        ...
        for learning_rate, max_depth, subsample, max_features in param_gridsearch:
             ...
             ## look at loss function for each the parameter set
    ...
    ## Defining our GBR models (use values obtained from tuning above)
    if(name == 0):
        ...
        model_c2_crossVal_huber[name] = GradientBoostingRegressor(n_estimators=200, learning_rate=0.1, max_depth=2, subsample = 1.0, max_features = 1.0, random_state=0, loss='huber').fit(X_train, y_train)
        model_c2_crossVal_ls[name] = GradientBoostingRegressor(n_estimators=200, learning_rate=0.1, max_depth=4, subsample = 1.0, max_features = 0.7, random_state=0, loss='ls').fit(X_train, y_train)
    ...
    if(name == 0):
        ...
        paramXGB_huber['eta']              	= 0.1 # learning rate
        paramXGB_huber['max_depth']        	= 4  # maximum depth of a tree
        paramXGB_huber['subsample']        	= 1.0 # fraction of events to train tree on
        paramXGB_huber['colsample_bytree'] 	= 0.8 # fraction of features to train tree on
        paramXGB_huber['n_estimators'] = 200

        paramXGB_ls['eta']              	= 0.07 # learning rate
        paramXGB_ls['max_depth']        	= 2  # maximum depth of a tree
        paramXGB_ls['subsample']        	= 1.0 # fraction of events to train tree on
        paramXGB_ls['colsample_bytree'] 	= 0.7 # fraction of features to train tree on
        paramXGB_ls['n_estimators'] = 200
    ...
    ## Custom rates (studying decreasing evolving learning rate)
    ## First defining custom rates (learning rates for a certain number of iterations)
    ## Five set of combinations defined below used for the study
    custom_rates = [1]*200
    learningrateComb =[[[0.3]*20,[0.1]*20,[0.07]*160],
                [[0.1]*20,[0.07]*20,[0.05]*160],
                [[0.1]*20,[0.07]*180],
                [[0.1]*20,[0.05]*180],
                [[0.07]*20,[0.05]*180]]
    LRtext = ["0.3(0-20),0.1(20-40),0.07(>40)",
              "0.1(0-20),0.07(20-40),0.05(>40)", 
              "0.1(0-20),0.07(>20)",
              "0.1(0-20),0.05(>20)", 
              "0.07(0-20),0.05(>20)"]

    for l in range(len(learningrateComb)):
        if(l == 0):
            fig, ax = plt.subplots(num='performance', figsize=(18,16))
        custom_rates = []
        for i in range(len(learningrateComb[l])):
            custom_rates.extend(learningrateComb[l][i])
        print('custom_rates = ',custom_rates)
        output = 'targetMul' 
        progress = dict()
        watchlist = [(test, 'eval'), (train, 'train')]
        ntrees = paramXGB_huber['n_estimators']
        #    model_c3_huber[name] = xgb.train(paramXGB_huber, train, ntrees, watchlist, evals_result=progress)
        model_c3_huber[name] = xgb.train(paramXGB_huber, train, ntrees, watchlist, evals_result=progress, callbacks=[xgb.callback.reset_learning_rate(custom_rates)])
        ## retrieve performance metrics
        epochs = len(progress['train']['mphe']) ## mphe, rmse
        x_axis = range(0, epochs)
        ## uncomment to plot loss performance vs overtraining 
        reldiff = [0]*epochs 
        for p in range(len(progress['train']['mphe'])):
            reldiff[p] = (progress['eval']['mphe'][p] - progress['train']['mphe'][p])/(progress['train']['mphe'][p])
        ax.plot(reldiff, progress['eval']['mphe'], label=LRtext[l])
        ax.tick_params(axis='both', which='major', labelsize=20)
        ax.legend(frameon=False,fontsize=20)
        plt.ylabel('Test error',fontsize=20)
        plt.xlabel('Overtraining [(test error - train error)/train error]',fontsize=20)
        plt.title('XGBoost Overtraining , '+fe_names[name])
        plt.savefig(plotdir+'xgboost_training_deviance_huber_'+pileup+'_'+clustering+'_'+fe_names[name]+'.png')
        plt.savefig(plotdir+'xgboost_training_deviance_huber_'+pileup+'_'+clustering+'_'+fe_names[name]+'.pdf')
        ## uncomment to plot train and test loss comparison
...
#        plt.xlabel('Boosting Iterations',fontsize=20)
#        plt.ylabel('Error',fontsize=20)
#        plt.title('XGBoost Loss , '+fe_names[name]+' LR:'+LRtext[l],fontsize=20)
#        plt.savefig(plotdir+'xgboost_training_deviance_huber_'+pileup+'_'+clustering+'_'+fe_names[name]+'_'+str(l)+'.png')
#        plt.savefig(plotdir+'xgboost_training_deviance_huber_'+pileup+'_'+clustering+'_'+fe_names[name]+'_'+str(l)+'.pdf')

    ntrees = paramXGB_ls['n_estimators']
    model_c3_ls[name] = xgb.train(paramXGB_ls, train, ntrees, watchlist, evals_result=progress, callbacks=[xgb.callback.reset_learning_rate(custom_rates)])
