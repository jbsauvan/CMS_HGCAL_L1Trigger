...
tuningGBR = 'True' # set to True if performing hyperparameter scan for GradientBoostingRegressor
tuningXGBOOST = 'False' # set to True if performing hyperparameter scan for XGBOOST
lossfuncdir = 'lad' # huber or ls or final (output directory names used for storing plots 
...
for name in df_eg_train:
    ...
    if(tuningXGBOOST == 'True'):
        ...
        for eta, max_depth, subsample, colsample_bytree in gridsearch_params:
            ...
            epochs = len(progress['train']['mphe']) ## mphe, rmse
            x_axis = range(0, epochs)
            # plot loss function for each hyperparamter set
            fig, ax = plt.subplots(figsize=(12,12))
            ax.plot(x_axis, progress['train']['mphe'], label='Train')
            ax.plot(x_axis, progress['eval']['mphe'], label='Test')
    ...
    #### Defining the final models (values obtained from tuning)
    if(name == 0):
