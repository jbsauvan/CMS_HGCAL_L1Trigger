#!/usr/bin/env python
# coding: utf-8
#### Preprocessing script to select events from the HGCAL TPG ntuples. This selects events with generated particle matched to a 3D cluster within a threshold radius. The script stores all events inside and outside the threshold radius (updated in comparison to the previous preprocessing script). Events having matched clusters are stored with 'best_match' = True. 
...
def preprocessing(path, files, savedir,  thr):
    ...
    for i in algo:
        ...
        print('gen pos events',(gen_pos).shape[0])
        print('gen neg events',(gen_neg).shape[0])
        print('algo pos events',(algo_pos).shape[0])
        print('algo neg events',(algo_neg).shape[0])
        ...
        print('algo pos gen events',(algo_pos_merged).shape[0])
        print('algo neg gen events',(algo_neg_merged).shape[0])
        ...
        print('algo pos gen dR events',(algo_pos_merged_dR).shape[0])
        print('algo neg gen dR events',(algo_neg_merged_dR).shape[0])
        ...
        print('algo pos gen Matching events',(algo_pos_merged).shape[0])
        print('algo neg gen Matching events',(algo_neg_merged).shape[0])
...
#        algo_neg_merged=algo_neg_merged[sel]
 ...
 #       algo_clean[i].drop(columns=['best_match'], inplace=True)
        ...
        print('algo gen all events',(algo_clean[i]['genpart_pt']).shape[0])
        print('algo gen M all events',(algo_clean_matched[i]['genpart_pt']).shape[0])
